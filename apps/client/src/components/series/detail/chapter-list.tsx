...
export function ChapterList({ chapters, seriesId }: ChapterListProps) {
  ...
  // Strong filter: only chapters in the active language with readable pages
  const validChapters = (chapters || []).filter((ch) => {
    const isLangMatch =
      locale === 'es'
        ? ch.translatedLanguage === 'es' || ch.translatedLanguage === 'es-la'
        : ch.translatedLanguage === 'en';
    return isLangMatch && (ch.pages ?? 0) > 0;
  });
    ...
    <ScrollArea className="h-[650px] w-full border-t border-dark-800 bg-transparent pt-4 pr-3">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-2 pb-4">
        {validChapters.map((chapter) => {
          const chapterNumber = chapter.chapter || t('extra');
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="flex flex-col min-w-0">
                  <span className="text-text-primary font-semibold text-sm group-hover:text-accent transition-colors truncate">
                    {t('episodeNumber', { number: chapterNumber })}
                    {displayTitle}
                  </span>
                  <div className="flex items-center gap-2 text-text-muted text-[11px] mt-0.5">
                    <span>{formattedDate}</span>
                    <span className="w-1 h-1 rounded-full bg-dark-600" />
                    <span className="uppercase font-bold text-text-secondary">
